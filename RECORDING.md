...
| 3:35–4:15 | Finish | The house-rules manifesto and the review summary (tap an Edit to show it jumping back and returning). **Cross-field logic:** pick Karnataka → Bengaluru → PES University, then change the state and show city *and* college both clear. Pick fewer than 3 vibes → error. **Numeric-only input:** paste `+91 98765-43210` into the mobile field and show it normalise to 10 digits. Set the name to include `errortest` and Sign Up → **server error toast, form intact**, then retry → success screen. |
